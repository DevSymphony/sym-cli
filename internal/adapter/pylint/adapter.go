...
// Adapter wraps Pylint for Python static analysis.
//
...
// NewAdapter creates a new Pylint adapter.
...
// GetCapabilities returns the Pylint adapter capabilities.
...
func (a *Adapter) Install(ctx context.Context, config adapter.InstallConfig) error {
	...
	venvPath := a.getVenvPath()
	pipPath := a.getPipPath()
	pylintPath := a.getPylintPath()
	...
	if _, err := os.Stat(venvPath); os.IsNotExist(err) {
		output, err := a.executor.Execute(ctx, pythonCmd, "-m", "venv", venvPath)
		if err != nil {
	...
	if _, err := os.Stat(pipPath); os.IsNotExist(err) {
		pythonInVenv := a.getPythonInVenv()
		output, err := a.executor.Execute(ctx, pythonInVenv, "-m", "ensurepip", "--upgrade")
		if err != nil {
	...
	output, err := a.executor.Execute(ctx, pipPath, "install", fmt.Sprintf("pylint%s", version))
	if err != nil {
...
func (a *Adapter) getVenvPath() string {
	return filepath.Join(a.ToolsDir, "pylint-venv")
}

// getPylintPath returns the path to local Pylint binary.
func (a *Adapter) getPylintPath() string {
	venvPath := a.getVenvPath()
...
func (a *Adapter) getPipPath() string {
	venvPath := a.getVenvPath()
	if runtime.GOOS == "windows" {
...
func (a *Adapter) getPythonCommand() string {
...
func (a *Adapter) getPythonInVenv() string {
	venvPath := a.getVenvPath()
	if runtime.GOOS == "windows" {
...
func (a *Adapter) getPylintCommand() string {
